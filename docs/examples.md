...
执行归零流程：关闭扭矩 → 手动拖动 → 设置零点 → 开启扭矩。

自 v6.0.0 起，SDK 会自动记录当前关节角度并保存为“软件零点”，下次连接同一机械臂时会自动加载，让 0°/0rad 对应刚才标定的姿态。

> 提示：归零后 `get_joints()` 会围绕新的零位显示 0，但 `get_pose()` 和所有 IK 计算仍在机器人模型（URDF）坐标系内工作，因此末端 XYZ/姿态值会继续保持出厂参考（例如直立姿态约 `[0, 0, 0.63]`）。
